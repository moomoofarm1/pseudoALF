...
Validation design
-----------------
For the selected human-annotated source files:
  ...
  3. Use each file once as the support file and all remaining files as query files.
  ...
  5. Report exact entity-level precision, recall and F1 in the GDPR-ALF target schema.
...
SCRIPT_VERSION = "2026-09-04-fewshot-pseudonymization-loo-v2-gdpr-alf"
...
# Exact GDPR-ALF target schema from the supplied PDF. Parent labels that also
# occur in the child column (PER, LOC, ORG, etc.) are valid child-level targets.
# MISC has no child tag in the table, but is retained as the PDF's catch-all
# category for personal information not covered by another category.
...
# Legacy labels seen in earlier annotation/alignment runs. Destinations are
# always GDPR-ALF targets. NUM, PERCENT and RELATION are not target labels in
# the supplied PDF, so already-sensitive instances are folded into MISC.
...
def align_gold_to_presidio(
    ...
    """Align gold and Presidio records by physical source-line number."""
    gold_by_line = {g.line_no: g for g in gold_records}
    pres_by_line = {line_no: txt for line_no, txt in presidio_records}
    if len(gold_by_line) != len(gold_records):
        raise ValueError("Duplicate gold record line numbers detected.")
    if len(pres_by_line) != len(presidio_records):
        raise ValueError("Duplicate Presidio record line numbers detected.")

    gold_lines = set(gold_by_line)
    pres_lines = set(pres_by_line)
    if gold_lines != pres_lines:
        missing = sorted(gold_lines - pres_lines)
        extra = sorted(pres_lines - gold_lines)
        raise ValueError(
            "Gold and Presidio files do not contain the same model-text line "
    ...
    for line_no in sorted(gold_lines):
        g = gold_by_line[line_no]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Require this many selected gold files; 0 disables the count check.",
    )
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON mapping raw/legacy gold labels into the supplied "
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Check files/model, align gold to Presidio and write audits, but "
            "skip transformer embeddings and few-shot evaluation."
        ),
    )
    return p
...
def load_aliases(args) -> Dict[str, str]:
    aliases = _validated_target_mapping(
        DEFAULT_GOLD_ALIASES, "DEFAULT_GOLD_ALIASES"
    )
    if args.label_map_json is not None:
        ...
        aliases.update(
            _validated_target_mapping(custom, "--label-map-json")
        )
    return aliases
...
def main() -> int:
    ...
    print("GDPR-ALF target labels (evaluation space):")
    ...
    for gold_path in gold_paths:
        ...
        file_alignment_summary.append({
            ...
            "out_of_schema_gold_entities": status_counts[
                "out_of_gdpr_alf_schema"
            ],
    ...
    summary = {
        ...
        "scopes": {
            ...
            "all_schema": (
                "Whole supplied GDPR-ALF target schema. For ProtoBERT/StructShot, "
        ...
        "gold_to_gdpr_alf_aliases": aliases,
        "files": [
            {
                "name": b.name,
