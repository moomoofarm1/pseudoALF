...
Designed for the vdeidspacy conda environment and the spaCy model:
    da_dacy_large_ner_fine_grained
(Hugging Face model card: chcaa/da_dacy_large_ner_fine_grained)

Validation design
-----------------
For six human-annotated source files:
  1. Use the Presidio-preprocessed counterpart as the ONLY text seen by the model.
  2. Transfer only gold spans that survived Presidio unchanged onto the
     preprocessed text. Gold spans changed/removed by Presidio are audited and
     excluded from residual DaCy evaluation.
  3. Use each file once as the support file and the other five as query files.
     Fold order is shuffled reproducibly, but every file is support exactly once.
  4. Evaluate:
       - dacy_zero_shot: the installed DaCy NER model as a no-adaptation baseline.
       - protobert: ProtoBERT-style class prototypes over frozen DaCy transformer
         token representations, Euclidean distance.
       - structshot: StructShot-style nearest-neighbour emissions + an abstract
         IO transition matrix + Viterbi decoding.
  5. Report exact entity-level precision, recall and F1.

Important methodological note
-----------------------------
The original StructShot method estimates its abstract transition statistics from
source-domain labeled data. This script intentionally avoids using query labels
or any external target labels. Because the DANSK source training labels are not
provided here, the abstract transition statistics are estimated from the single
support file with smoothing. The nearest-neighbour + structured decoding logic
is StructShot; the transition-statistics source is therefore "support-only".

The "one support file" design is NOT literally 1-shot per entity class. Each
support file can contain a variable number of mentions per class. The script
writes support_shots.csv so the effective number of mentions/tokens per label is
explicit for every fold.

Only the quoted utterance is passed to the model for .alfrttm files. Timing and
speaker metadata are never model input.
"""
...
SCRIPT_VERSION = "2026-09-04-fewshot-pseudonymization-loo-v1"

# The model card reports these 18 fine-grained labels. At runtime we use the
# installed model's actual ner.labels and only use this list as a sanity check.
...
# Unambiguous aliases encountered in the existing pseudonymization pipeline.
# LOC is deliberately NOT mapped automatically because DaCy distinguishes
# LOCATION and GPE. Use --map-loc-to-location only if your gold LOC truly means
# DaCy LOCATION rather than a collapsed parent category.
...
def extract_utterance(line: str, strict: bool, path: Path, line_no: int) -> Optional[str]:
    """Return quoted utterance from one ALFRRTM line."""
    first = line.find('"')
    last = line.rfind('"')
    ...
    if strict:
        if line.strip():
            raise ValueError(
                ...
                "Use well-formed ALFRRTM input or omit --strict-quoted-utterance."
            )
        return ""
    return line.rstrip("\r\n")

def parse_gold_utterance(
    utterance: str,
    ann_re: re.Pattern,
...
def load_gold_records(
    ...
    for i, line in enumerate(lines, start=1):
        ...
        plain, entities = parse_gold_utterance(utterance, ann_re, aliases)
        records.append(
            GoldRecord(
...
def map_entity_through_presidio(
    ...
    """
    Map a gold span only when the whole entity survived unchanged.
...
def align_gold_to_presidio(
    gold_records: List[GoldRecord],
    presidio_records: List[Tuple[int, str]],
    model_labels: set,
) -> Tuple[List[ResidualRecord], List[dict]]:
    if len(gold_records) != len(presidio_records):
        raise ValueError(
            "Gold and Presidio files have different record counts: "
            f"{len(gold_records)} vs {len(presidio_records)}. "
            "The pipeline assumes Presidio preserves line/utterance order."
        )

    residual: List[ResidualRecord] = []
    audit_rows: List[dict] = []

    for g, (p_line_no, p_text) in zip(gold_records, presidio_records):
        kept: List[Entity] = []
        dropped: List[Entity] = []
        unsupported: List[Entity] = []

        for ent in g.raw_entities:
            if ent.label not in model_labels:
                unsupported.append(ent)
                audit_rows.append({
                    "record_uid": g.uid,
                    "gold_line_no": g.line_no,
                    "presidio_line_no": p_line_no,
                    "raw_label": ent.raw_label,
                    "canonical_label": ent.label,
                    "entity_text": ent.text,
                    "status": "unsupported_by_dacy",
                })
                continue

            mapped = map_entity_through_presidio(
                g.original_plain, p_text, ent
            )
            if mapped is None:
                dropped.append(ent)
                audit_rows.append({
...
def match_presidio_file(gold_path: Path, presidio_candidates: List[Path]) -> Path:
    same_name = [
        p for p in presidio_candidates
        if p.name.casefold() == gold_path.name.casefold()
    ]
    if len(same_name) == 1:
        return same_name[0]
    if len(same_name) > 1:
        raise ValueError(
            f"Multiple Presidio files match basename {gold_path.name}: "
            + ", ".join(map(str, same_name))
        )

    gkey = normalized_file_key(gold_path)
    same_key = [
        p for p in presidio_candidates
        if normalized_file_key(p) == gkey
        and p.suffix.lower() == gold_path.suffix.lower()
    ]
    if len(same_key) == 1:
        return same_key[0]
    if len(same_key) > 1:
    ...
    raise FileNotFoundError(
        f"No Presidio counterpart found for: {gold_path}"
    )
...
def token_vectors_from_doc(doc) -> np.ndarray:
    """
    Mean-pool transformer wordpieces aligned to each spaCy token.

    Compatible with spaCy 3.5 + spacy-transformers 1.2 style
    Doc._.trf_data.
    """
    if not hasattr(doc._, "trf_data"):
        raise RuntimeError(
            "Doc._.trf_data is unavailable. The loaded model must include "
            "a spacy-transformers 'transformer' component."
        )
    ...
    if model_output is not None:
        ...
        if hidden is None and isinstance(model_output, dict):
            hidden = model_output.get("last_hidden_state")

    if hidden is None:
        # Backward-compatible fallback.
        for tensor in getattr(data, "tensors", ()):
    ...
    if hidden is None:
        raise RuntimeError(
            "Could not locate transformer last hidden states in Doc._.trf_data."
        )

    hidden = _to_numpy(hidden).astype(np.float32, copy=False)
    if hidden.ndim == 3:
        ...
        raise RuntimeError(
            f"Unexpected transformer hidden-state shape: {hidden.shape}"
        )
    ...
    for i in range(len(doc)):
        aligned = _to_numpy(data.align[i].dataXd).astype(
            np.int64, copy=False
        ).ravel()
...
def encode_bundle(
    ...
    texts = [r.text for r in bundle.records]
...
def gold_token_labels(
    record: EncodedRecord,
) -> Tuple[List[str], int]:
    labels = ["O"] * len(record.tokens)
    boundary_expansions = 0

    for ent in record.gold_entities:
        overlapping = []
        exact_left = False
        exact_right = False
        ...
        for i in overlapping:
            if labels[i] != "O" and labels[i] != ent.label:
                raise ValueError(
                    f"Overlapping gold entities with conflicting labels in "
...
def build_support(
    ...
    for rec in support_bundle.encoded:
        labels, n_expanded = gold_token_labels(rec)
        boundary_expansions += n_expanded
        label_sequences.append(labels)

        for ent in rec.gold_entities:
...
def score_counters(gold: Counter, pred: Counter) -> dict:
...
def aggregate_average_metrics(fold_rows: Sequence[dict]) -> List[dict]:
    ...
    for (method, scope), rows in sorted(groups.items()):
        ...
        # Pool the entity-level contingency counts across the six LOO folds.
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Leave-one-file-out few-shot validation using the DaCy fine-grained "
            "NER model as frozen feature extractor."
        )
    )
    p.add_argument("--version", action="version", version=SCRIPT_VERSION)

    p.add_argument("--gold-dir", type=Path, required=True)
    p.add_argument("--presidio-dir", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, required=True)

    p.add_argument(
        "--model",
        default="da_dacy_large_ner_fine_grained",
        help="Installed spaCy pipeline name.",
    )
    p.add_argument(
        "--extensions",
        nargs="+",
    ...
    p.add_argument(
        ...
        default=[],
        help=(
            "Optional filename substrings. If provided, only gold files whose "
            "basename contains at least one substring are used."
        ),
    )
    p.add_argument("--expected-files", type=int, default=6)
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Off by default to stay closer to raw Euclidean ProtoBERT logic."
        ),
    )
    ...
    p.add_argument(
        ...
        default=2048,
        help=(
            "Maximum O-class support tokens kept for StructShot nearest-neighbour "
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Map gold LOC -> DaCy LOCATION. Off by default because LOC may be "
            "a collapsed parent over LOCATION and GPE."
        ),
    )
    p.add_argument(
        "--label-map-json",
        ...
        default=None,
        help=(
            "Optional JSON object mapping gold labels to DaCy labels, e.g. "
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "Check files, load the model, align gold to Presidio and write "
            "alignment audits, but skip embeddings/few-shot evaluation."
        ),
    )
    return p

def load_aliases(args) -> Dict[str, str]:
    aliases = dict(DEFAULT_GOLD_ALIASES)
    if args.map_loc_to_location:
        aliases["LOC"] = "LOCATION"
    if args.label_map_json is not None:
        with args.label_map_json.open("r", encoding="utf-8") as f:
            custom = json.load(f)
        if not isinstance(custom, dict):
            raise ValueError("--label-map-json must contain a JSON object.")
        for k, v in custom.items():
            aliases[str(k).strip().upper()] = str(v).strip().upper().replace(
                "WORK_OF_ART", "WORK OF ART"
            )
    return aliases
...
def load_spacy_model(args):
    ...
    return nlp, model_labels, used_gpu, raw_labels
...
def main() -> int:
    ...
    if not args.gold_dir.is_dir():
        raise FileNotFoundError(f"Gold directory not found: {args.gold_dir}")
    if not args.presidio_dir.is_dir():
        raise FileNotFoundError(
            f"Presidio directory not found: {args.presidio_dir}"
        )

    gold_paths = discover_files(
        args.gold_dir,
        extensions=extensions,
        include_substrings=args.include_substrings,
    )
    # if args.expected_files > 0 and len(gold_paths) != args.expected_files:
    #     raise RuntimeError(
    #         f"Expected {args.expected_files} gold files, found {len(gold_paths)}:\n"
    #         + "\n".join(map(str, gold_paths))
    #     )

    presidio_candidates = discover_files(
        args.presidio_dir,
        extensions=extensions,
        include_substrings=[],
    )
    if not presidio_candidates:
        raise RuntimeError(
            f"No Presidio files found under {args.presidio_dir}"
        )

    print(f"Script version: {SCRIPT_VERSION}")
    print(f"Gold files: {len(gold_paths)}")
    for pth in gold_paths:
        print(f"  - {pth}")

    print("\nLoading DaCy model...")
    nlp, model_labels, used_gpu, raw_model_labels = load_spacy_model(args)
    print(f"Model: {args.model}")
    print(f"GPU active: {used_gpu}")
    print("Model NER labels:")
    ...
    file_alignment_summary: List[dict] = []
    raw_gold_entity_count = 0

    for gold_path in gold_paths:
        presidio_path = match_presidio_file(
            gold_path, presidio_candidates
        )
        ...
        raw_gold_entity_count += sum(
            len(record.raw_entities) for record in gold_records
        )
        pres_records = load_presidio_records(
            presidio_path,
            strict_quoted_utterance=args.strict_quoted_utterance,
        )
        residual_records, audit = align_gold_to_presidio(
            gold_records,
            pres_records,
        ...
        for row in audit:
            row = dict(row)
            row["file"] = gold_path.name
            alignment_audit.append(row)

        status_counts = Counter(r["status"] for r in audit)
        supported_total = (
            status_counts["survived_presidio"]
            + status_counts["changed_or_removed_by_presidio"]
        )
        survived = status_counts["survived_presidio"]

        file_alignment_summary.append({
            "file": gold_path.name,
            "gold_path": str(gold_path),
            "presidio_path": str(presidio_path),
            "records": len(residual_records),
            "supported_gold_entities_before_presidio": supported_total,
            "supported_gold_entities_survived_presidio": survived,
            "supported_gold_entities_changed_or_removed": status_counts[
                "changed_or_removed_by_presidio"
            ],
            "unsupported_gold_entities": status_counts[
                "unsupported_by_dacy"
            ],
            "supported_survival_rate": (
                survived / supported_total if supported_total else 0.0
            ),
        })
        ...
        bundles.append(
            FileBundle(
                name=gold_path.name,
                gold_path=gold_path,
                presidio_path=presidio_path,
                records=residual_records,
            )
        )

    if raw_gold_entity_count == 0:
        raise RuntimeError(
            "No gold pseudonymization annotations were found in the selected "
            "files. The validator expects preserved annotated originals such "
            "as [PersonData]mention[LABEL]. Do not point --gold-dir at files "
            "that were destructively overwritten by the notebook's square-"
            "bracket removal step."
        )

    write_csv(
        args.output_dir / "presidio_alignment_audit.csv",
        alignment_audit,
    )
    write_csv(
        args.output_dir / "presidio_alignment_summary.csv",
        file_alignment_summary,
    )

    if args.preflight_only:
        print(
            "\nPreflight complete. Alignment audit written; evaluation skipped."
        )
        return 0

    print("\nEncoding Presidio-preprocessed utterances with DaCy...")
    ...
    for fold_number, support_idx in enumerate(support_order, start=1):
        ...
        boundary_rows.append({
            ...
            "support_gold_entities_not_exactly_on_spacy_token_boundaries": (
                boundary_expansions
            ),
        })
        ...
        fold_gold = {
            scope: Counter()
            for scope in ("support_observed", "all_supported")
        }
        fold_pred = {
        ...
        for query_bundle in query_bundles:
            ...
            for rec in query_bundle.encoded:
                ...
                for scope, allowed in allowed_by_scope.items():
                    ...
                    pair_gold[scope].update(g)
                    fold_gold[scope].update(g)
        ...
        for scope in ("support_observed", "all_supported"):
            ...
            for method in methods:
                ...
                for label in labels_for_scope:
                    sg = filter_counter_label(fold_gold[scope], label)
    ...
    summary = {
        ...
        "scopes": {
            "support_observed": (
                ...
                "one surviving gold example in the support file."
            ),
            "all_supported": (
                "Practical whole-model scope: all gold labels supported by the "
                "installed DaCy model. For ProtoBERT/StructShot, target labels "
                "absent from support cannot be predicted and therefore become "
                "false negatives."
            ),
        },
        "presidio_policy": (
            "Model text is always the Presidio-preprocessed counterpart. Gold "
            "entities changed/removed by Presidio are excluded from residual "
            "DaCy evaluation and listed in presidio_alignment_audit.csv."
        ),
        ...
        "files": [
            {
                "name": b.name,
                "gold_path": str(b.gold_path),
                "presidio_path": str(b.presidio_path),
            }
            for b in bundles
        ],
        "outputs": [
            ...
            "pair_metrics.csv",
            "support_shots.csv",
            "presidio_alignment_summary.csv",
            "presidio_alignment_audit.csv",
